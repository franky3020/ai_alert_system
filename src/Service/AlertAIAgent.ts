...
export class AlertAIAgent {
  ...
  constructor() {
    const exampleAlert = new UserAlertMsg(
      new Date("2021-07-27T18:24:00"),
      { latitude: 24.17129297887858, longitude: 120.59929167921332 },
      "user123",
      "商場內發現可疑包裹, 且外型像是炸彈"
    );
    ...
    const exampleAlert1 = new UserAlertMsg(
      new Date("2021-07-27T19:13:00"),
      { latitude: 24.257829981245745, longitude: 120.51848204285801 },
      "user777",
      "星星大樓 3樓 窗外出現黑煙"
    );
    this.messages.push({
  ...
  async getRiskAssessmentReport(
    ...
    if (resuleMsg != null) {
      let riskAssessmentReport: RiskAssessmentReport = JSON.parse(resuleMsg);

      const validLength = 3;
      let riskAssessmentReportLength = Object.keys(riskAssessmentReport).length;
      if (Object.keys(riskAssessmentReport).length !== validLength) {
        throw new Error(
          "the response not valid, length = " + riskAssessmentReportLength
        );
      }

      if (riskAssessmentReport.level === undefined) {
        throw new Error("level is undefined");
      }

      if (riskAssessmentReport.basisOfJudgment === undefined) {
        throw new Error("basisOfJudgment is undefined");
      }

      if (riskAssessmentReport.distanceRadiusOfInfluence === undefined) {
        throw new Error("distanceRadiusOfInfluence is undefined");
      }

      return new RiskAssessmentReport(
        riskAssessmentReport.level,
        riskAssessmentReport.distanceRadiusOfInfluence,
        riskAssessmentReport.basisOfJudgment
      );
    }
